...
def listen_print_loop(responses, stream):
    ...
    for response in responses:
        ...
        if not result.is_final:
            ...
            trans_result = subword_encoder_en.decode(predicted_seq_without_bos_eos)
    
            print(Fore.GREEN + Back.RED + '======== 你說： ========')
            print(transcript + overwrite_chars)
            print(Fore.BLUE + Back.YELLOW + '======== 我的翻譯： ========')
            print(trans_result)
            TTSspeak(trans_result, 'en-us')
            time.sleep(5)
            break
...
def _main():
    ...
    speak_text = "你好，我可以幫你把中文翻譯成英文。"
    print(speak_text)
    #TTSspeak(speak_text, 'zh-tw')
    #time.sleep(5)
